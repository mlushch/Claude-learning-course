...
async def test_get_tasks_with_status_filter(client):
    respx.get("http://localhost:5000/tasks", params={"status": "Open"}).mock(
        return_value=httpx.Response(200, json=[SAMPLE_TASK])
    )
...
async def test_get_tasks_raises_on_401(client):
    respx.get("http://localhost:5000/tasks").mock(return_value=httpx.Response(401))
...
async def test_get_task_returns_task(client):
    respx.get("http://localhost:5000/tasks/abc-123").mock(
        return_value=httpx.Response(200, json=SAMPLE_TASK)
    )
...
async def test_get_task_raises_on_404(client):
    respx.get("http://localhost:5000/tasks/missing").mock(
...
async def test_create_task_returns_created(client):
    ...
    respx.post("http://localhost:5000/tasks").mock(
...
async def test_create_task_raises_on_400(client):
    respx.post("http://localhost:5000/tasks").mock(return_value=httpx.Response(400))
    with pytest.raises(Exception):
...
async def test_update_task_returns_updated(client):
    ...
    respx.put("http://localhost:5000/tasks/abc-123").mock(
...
async def test_update_task_raises_on_404(client):
    respx.put("http://localhost:5000/tasks/missing").mock(
        return_value=httpx.Response(404)
    )
    with pytest.raises(Exception):
...
async def test_delete_task_succeeds(client):
    respx.delete("http://localhost:5000/tasks/abc-123").mock(
...
async def test_delete_task_raises_on_404(client):
    respx.delete("http://localhost:5000/tasks/missing").mock(
        return_value=httpx.Response(404)
    )
    with pytest.raises(Exception):
...
async def test_get_tasks_raises_on_500(client):
    respx.get("http://localhost:5000/tasks").mock(return_value=httpx.Response(500))
    with pytest.raises(Exception):
        await client.get_tasks()
